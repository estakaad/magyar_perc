...
export function useSyncedList(table, email) {
  ...
  const add = async (item) => {
    ...
    if (exists) return;
    const row = { word: item.word, translation: item.translation || '', note: item.note || '', email };
  ...
  const remove = async (word) => {
    await supabase.from(table).delete().eq('email', email).eq('word', word);
  ...
  const updateStats = async (word, field) => {
    ...
    await supabase.from(table).update({ [field]: newVal }).eq('email', email).eq('word', word);
  ...
  const updateExamples = async (word, examples) => {
    await supabase.from(table).update({ examples }).eq('email', email).eq('word', word);
